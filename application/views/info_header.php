...
// // get Fase from date now
  $dateNow = date('Y-m-d');
  $setDiff=date_diff(date_create($startDate),date_create($dateNow)); // menghitung selisih hari dari tanggal mulai dan tanggal sekarang
$dif = $setDiff->format("%a"); // difference date
$getDeadline = $this->m_site->getDeadline(); // mengambil daftar deadline setiap proses

$tot = 0;
// loop untuk mengambil total hari dari tanggal mulai hingga hari ini berdasarkan deadline hari di tiap fase
foreach ($getDeadline AS $g) {
 $tot += $g['WAKTU_PELAKSANAAN'];
 if($dif <= $tot){
  $fases = $g['ID_FASE']; // memasukkan data fase berdasarkan total hari
  break;
}}
$this->session->set_userdata('fase', $fases);  // menyimpan session untuk fase
